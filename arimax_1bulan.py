# ==================== COMPLETE ARIMAX IMPLEMENTATION - SOL 1 BULAN ====================
# FIXED VERSION - NO ERRORS
print("="*80)
print("ARIMAX PREDICTION FOR SOL 1 BULAN - METODE BENAR")
print("="*80)
...
query = """
    SELECT
        ...
        close,
        volume
    FROM `time-series-analysis-480002.SOL.SOL_1bulan`
    ORDER BY datetime
"""
...
plt.subplot(2, 2, 1)
plt.plot(df['close'].values, 'b-', linewidth=2)
...
# Plot 3: ACF
plt.subplot(2, 2, 3)
plot_acf(Y, lags=20, ax=plt.gca(), alpha=0.05)
plt.title('ACF Plot')
plt.xlabel('Lag')
...
# Plot 4: PACF
plt.subplot(2, 2, 4)
plot_pacf(Y, lags=20, ax=plt.gca(), alpha=0.05, method='ywm')
plt.title('PACF Plot')
plt.xlabel('Lag')
...
acf_values = acf(Y, nlags=10, fft=True)
pacf_values = pacf(Y, nlags=10, method='ywm')
...
# Karena Y sudah di-diff, kita mulai dari index 1 untuk Y dan index 2 untuk X
Y_aligned = Y  # Y_diff sudah mulai dari index 1
X_aligned = X[2:]  # Skip first 2 rows (1 untuk lag, 1 untuk align dengan Y_diff)
...
plt.plot(train_idx, Y_train, 'b-', linewidth=2, label='Training')
plt.plot(test_idx, Y_test, 'g-', linewidth=2, label='Test Actual')
plt.plot(test_idx, pred_mean, 'r--', linewidth=2, label='Forecast')
...
# Plot 2: Reconstructed prices (if d=1)
plt.subplot(2, 2, 2)

if d == 1:
    # Get actual prices for training and test
    ...
    plt.plot(full_train_idx, train_prices, 'b-', linewidth=2, label='Training')
    plt.plot(full_test_idx, test_prices, 'g-', linewidth=2, label='Test Actual')
    plt.plot(full_test_idx, pred_reconstructed, 'r--', linewidth=2, label='Forecast')
    ...
    # For d=0 or d=2, show differenced data
    plt.plot(train_idx, Y_train, 'b-', linewidth=2, label='Training')
    plt.plot(test_idx, Y_test, 'g-', linewidth=2, label='Test Actual')
    plt.plot(test_idx, pred_mean, 'r--', linewidth=2, label='Forecast')
    plt.axvline(x=len(Y_train), color='black', linestyle='--', alpha=0.7)
...
plt.plot(test_idx, residuals, 'o-', markersize=4, alpha=0.7)
...
plt.subplot(2, 2, 4)
plt.scatter(Y_test, pred_mean, alpha=0.6)
...
# ==================== 10. FUTURE FORECAST ====================
print("\n" + "="*80)
print("STEP 9: 1-MONTH AHEAD FORECAST")
print("="*80)

# Prepare exogenous for next period
if len(X_test) > 0:
    future_exog = X_test[-1:].reshape(1, -1)
else:
    future_exog = X_train[-1:].reshape(1, -1)

# Forecast 1 step ahead
try:
    future_forecast = best_fit.get_forecast(steps=1, exog=future_exog)
    future_pred_diff = future_forecast.predicted_mean

    # Handle different return types
    if hasattr(future_pred_diff, '__len__'):
        future_pred_diff = future_pred_diff[0]

    future_ci = future_forecast.conf_int()

    # Handle different return types for CI
    if hasattr(future_ci, 'values'):
        future_ci = future_ci.values[0]
    elif hasattr(future_ci, '__len__'):
        if len(future_ci) == 2:
            future_ci = future_ci
        else:
            future_ci = future_ci[0]
    else:
        future_ci = [future_pred_diff - 1.96 * rmse, future_pred_diff + 1.96 * rmse]

    # Reconstruct to original price
    last_price = df['close'].iloc[-1]

    if d == 0:
        future_pred_price = future_pred_diff
        future_ci_lower = future_ci[0] if isinstance(future_ci, (list, np.ndarray)) else future_ci
        future_ci_upper = future_ci[1] if isinstance(future_ci, (list, np.ndarray)) else future_ci
    elif d == 1:
        future_pred_price = last_price + future_pred_diff
        if isinstance(future_ci, (list, np.ndarray)) and len(future_ci) == 2:
            future_ci_lower = last_price + future_ci[0]
            future_ci_upper = last_price + future_ci[1]
        else:
            future_ci_lower = future_pred_price - 1.96 * rmse
            future_ci_upper = future_pred_price + 1.96 * rmse
    else:
        # For d=2, approximate
        future_pred_price = last_price + future_pred_diff
        future_ci_lower = future_pred_price - 1.96 * rmse * 2
        future_ci_upper = future_pred_price + 1.96 * rmse * 2
    ...
    print(f"  Forecast price: ${future_pred_price:.2f}")
    print(f"  Expected change: {((future_pred_price - last_price) / last_price * 100):+.2f}%")
    print(f"\n  95% Confidence Interval:")
    print(f"  Lower: ${future_ci_lower:.2f}")
    print(f"  Upper: ${future_ci_upper:.2f}")
    ...
    if d == 1 and len(pred_mean) > 0:
        future_pred_price = last_price + pred_mean[-1]
    else:
        future_pred_price = last_price

    future_ci_lower = future_pred_price * 0.95  # -5%
    future_ci_upper = future_pred_price * 1.05  # +5%

    last_date = df['datetime'].iloc[-1]
    forecast_date = last_date + pd.DateOffset(months=1)

    print(f"  Simple forecast: ${future_pred_price:.2f}")
...
def save_to_bigquery():
    ...
    # Get reconstructed prices if d=1, otherwise use original
    if d == 1:
        # Training prices
        train_prices = Y_original[1:1+len(Y_train)]
        ...
        # Test actual prices
        last_train_price = Y_original[len(train_prices)] if len(train_prices) < len(Y_original) else Y_original[-1]
        test_actual_prices = np.cumsum(Y_test) + last_train_price
        test_pred_prices = np.cumsum(pred_mean) + last_train_price

        # Test dates
        test_start = 1 + len(train_prices)
        test_end = min(test_start + len(test_actual_prices), len(df))
        test_dates = df['datetime'].iloc[test_start:test_end].tolist()

    else:
        # For d=0 or d=2, use original Y
        train_prices = Y_train
        train_dates = df['datetime'].iloc[:len(train_prices)].tolist()

        test_actual_prices = Y_test
        test_pred_prices = pred_mean
    ...
    for i in range(len(train_prices)):
        ...
        records.append({
            ...
            "accuracy": float(max(0, 100 - mape)) if not np.isnan(mape) else None,
            "mse": float(rmse**2),
            "rmse": float(rmse),
            "mae": float(mae),
    ...
    for i in range(len(test_actual_prices)):
        if i < len(test_dates):
            ...
            timestamp = timestamp + pd.DateOffset(months=i+1)
    ...
    for i in range(len(test_pred_prices)):
        ...
        if conf_int is not None and i < len(conf_int):
            if d == 1:
                ci_lower = test_pred_prices[i] - (test_pred_prices[i] - (last_train_price + conf_int[i, 0]))
                ci_upper = test_pred_prices[i] + ((last_train_price + conf_int[i, 1]) - test_pred_prices[i])
            else:
    ...
    # 4. FORECAST - Store 1-month ahead forecast
    ...
    records.append({
        ...
        "price_actual": None,
        "price_predicted": float(future_pred_price),
        "lower_ci": float(future_ci_lower),
        "upper_ci": float(future_ci_upper),
        "data_type": "FORECAST",
        "model_type": "ARIMAX",
        "timeframe": "1bulan",
        "training_date": training_date,
    ...
    print(f"  FORECAST: 1 record")

    # Calculate total expected records
    total_expected = len(train_prices) + len(test_actual_prices) + len(test_pred_prices) + 1
...
if not np.isnan(mape):
    ...
    print(f"  Accuracy (prices): {max(0, 100 - mape):.2f}%")
print(f"  RMSE (differences): {rmse:.4f}")
print(f"  MAE (differences): {mae:.4f}")

print(f"\n🔮 FUTURE FORECAST:")
print(f"  Date: {forecast_date.date()}")
print(f"  Forecast: ${future_pred_price:.2f}")
print(f"  Change: {((future_pred_price - last_price) / last_price * 100):+.2f}%")
print(f"  95% CI: [${future_ci_lower:.2f}, ${future_ci_upper:.2f}]")

print(f"\n💾 DATA STORAGE:")
print(f"  Status: {'✅ Successfully saved to BigQuery' if save_success else '❌ Failed to save'}")
print(f"  Table: arimax_1bulan in PREDIKSI dataset")
print(f"  Data types: TRAIN_ACTUAL, TEST_ACTUAL, TEST_PREDICTION, FORECAST")

print("\n" + "="*80)
print("ARIMAX ANALYSIS COMPLETE!")
print("="*80)