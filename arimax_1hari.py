# ==================== COMPLETE ARIMAX IMPLEMENTATION - SOL 1 HARI ====================
# FIXED VERSION - USING LAST 4 YEARS DATA, 1-DAY AHEAD FORECAST
print("="*80)
print("ARIMAX PREDICTION FOR SOL 1 HARI - DATA 4 TAHUN TERAKHIR")
print("="*80)
...
# ==================== 2. LOAD DATA (LAST 4 YEARS) ====================
print("\n" + "="*80)
print("STEP 1: LOADING DATA FROM BIGQUERY (4 YEARS)")
print("="*80)
...
# Calculate date range for last 4 years
end_date = datetime.now()
start_date = end_date - timedelta(days=4*365)  # Approximately 4 years

# Format dates untuk DATETIME (bukan TIMESTAMP)
lookback_date = start_date.strftime('%Y-%m-%d')

# Query data for last 4 years - SAMA PERSIS dengan LSTM
query = f"""
    SELECT 
        datetime,
        open,
        ...
        close,
        volume
    FROM `time-series-analysis-480002.SOL.SOL_1hari`
...
if d == 1:
    ...
    # This is simplified - for d=2, reconstruction is more complex
...
if d == 0:
    # For d=0, use original prices
    train_prices = Y_original[:len(Y_train)]
    test_prices = Y_test_actual_prices
    pred_reconstructed = pred_prices
else:
    # For d>=1, use reconstructed prices
    train_prices = Y_original[1:1+len(Y_train)] if d == 1 else Y_original[2:2+len(Y_train)]
    test_prices = Y_test_actual_prices
    pred_reconstructed = pred_prices
...
# ==================== 10. 1-DAY AHEAD FORECAST ====================
print("\n" + "="*80)
print("STEP 9: 1-DAY AHEAD FORECAST")
print("="*80)

# Prepare exogenous for next day
if len(X_test) > 0:
...
def save_to_bigquery():
    ...
    # Table name - CHANGED to sarimax_1hari
    table_name = "arimax_1hari"
    ...
    for i in range(len(test_actual_prices)):
        if i < len(test_dates):
            ...
            timestamp = timestamp + timedelta(days=i+1)
    ...
    # 4. FORECAST - Store 1-day ahead forecast
...
print(f"  Data period: Last 4 years ({start_date.date()} to {end_date.date()})")
...
print(f"  Table: sarimax_1hari in PREDIKSI dataset")
print(f"  Data types: TRAIN_ACTUAL, TEST_ACTUAL, TEST_PREDICTION, FORECAST")
print(f"  Data years used: 4")

print("\n" + "="*80)
print("ARIMAX ANALYSIS FOR 1-HARI COMPLETE!")
print("="*80)